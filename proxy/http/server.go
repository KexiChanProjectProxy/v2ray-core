...
func verifyCredentials(username, password string) error {
	// Split the username into uuid and timestamp parts (using the last hyphen)
	index := strings.LastIndex(username, "-")
	if index < 0 {
		return errors.New("invalid username format: missing hyphen separator")
	}
	uuidPart := username[:index]
	tsStr := username[index+1:]
	// Optionally, check that the uuid part is of length 36.
	if len(uuidPart) != 36 {
		return errors.New("invalid UUID format in username")
	}
	...
	// Compute the expected hash: SHA1(username + salt)
	h := sha1.New()
	if _, err := io.WriteString(h, username+salt); err != nil {
...
Start:
	...
	if len(s.config.Accounts) > 0 {
		...
		if !ok {
			errors.New("failed to parse basic headers")
        ...
        if err := verifyCredentials(user, pass); err != nil {
			errors.New("failed to verify credits)
            return common.Error2(conn.Write([]byte("HTTP/1.1 407 Proxy Authentication Required\r\nProxy-Authenticate: Basic realm=\"proxy\"\r\n\r\n")))
        }
		if inbound != nil {
